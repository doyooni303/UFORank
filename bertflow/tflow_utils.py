...
def simple_pooling(hidden_states, attention_mask, pooling_method):
    if "cls" == pooling_method:
        ...
        semb = (
            (hidden_states * attention_mask.unsqueeze(-1)).sum(dim=1).div(lengths)
        )  # (bsz, hdim)
    elif "max" == pooling_method:
        assert attention_mask is not None
        mask = (
            1 - attention_mask.unsqueeze(dim=-1)
        ).bool()  # (bsz, seq_len) -> (bsz, seq_len, 1)
...
class ActNorm(nn.Module):
    def __init__(self, in_channel, logdet=True, logscale_factor=3.0, ddi=True):
        super().__init__()
    ...
    def initialize(self, input):
        ...
        with torch.no_grad():
            flatten = (
                input.permute(1, 0, 2, 3).contiguous().view(input.shape[1], -1)
            )  # (hdim, bsz)
            mean = flatten.mean(1)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.split:
            ...
            log_p = prior_dist.log_prob(x2).sum_to_size(b_size, 1, 1, 1).view(b_size)
...
class Glow(nn.Module):
    ...
    def forward(self, emb):
        ...
        prior_objective = (
            prior_dist.log_prob(z).sum_to_size(b_size, 1, 1, 1).view(b_size)
        )
...
class TransformerGlow(nn.Module):
    def __init__(self, device0=None, device1=None, model_name_or_path=None, pooling=None):
        super().__init__()
        ...
        if self.device1 is not None:
            self.transformer = AutoModel.from_pretrained(model_name_or_path).to(self.device0)
            self.glow = Glow(self.transformer.config.hidden_size).to(self.device1)
        else:
            self.transformer = AutoModel.from_pretrained(model_name_or_path).to(self.device0)
            self.glow = Glow(self.transformer.config.hidden_size).to(self.device0)
        ...
        with torch.no_grad():
            if self.pooling == "first-last-avg":
                ...
                semb = (
                    ...
                    (all_hidden_states[-0] * attention_mask.unsqueeze(-1)).sum(
                        dim=1
                    )
                    + (all_hidden_states[-1] * attention_mask.unsqueeze(-1)).sum(
                        dim=1
                    )
                ).div(
                    2 * lengths
                ...
                semb = simple_pooling(
                    hidden_states, attention_mask, self.pooling
                )  # (bsz, hdim)

        if self.device1 is not None:
            z, loss = self.glow(semb.to(self.device1))
        else:
